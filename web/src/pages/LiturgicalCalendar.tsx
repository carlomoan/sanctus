...
import { Calendar, ChevronLeft, ChevronRight, Church, Palette, Clock, Info, Plus, Edit, Trash2, Eye, Filter, Search, Star, Cross, Heart, Flame, Bell, BookOpen, Users, MapPin, X } from 'lucide-react';
...
const LiturgicalCalendar = () => {
  ...
  useEffect(() => {
    const mockLiturgicalDays: LiturgicalDay[] = [
      {
        ...
        description: 'Christmas Day - The birth of our Lord Jesus Christ',
      ...
      },
      {
        ...
        description: 'Patronal feast of the United States',
        feast_type: 'SOLEMNITY',
      ...
      },
      {
        ...
        date: `${year}-04-09`,
        title: 'Good Friday',
        description: 'Crucifixion of the Lord',
        feast_type: 'SOLEMNITY',
        liturgical_season: 'HOLY_WEEK',
    ...
    const mockPatterns: RecurringPattern[] = [
      {
        ...
        pattern_type: 'weekly',
        pattern_config: { day_of_week: 0, time: '09:00' },
        is_active: true,
        created_at: '2024-01-01T00:00:00Z',
        updated_at: '2024-01-01T00:00:00Z'
      },
      {
        id: '2',
        name: 'Daily Mass',
        description: 'Daily Mass celebration',
        pattern_type: 'weekly',
        pattern_config: { day_of_week: 1, time: '08:00' },
        is_active: true,
        created_at: '2024-01-01T00:00:00Z',
        updated_at: '2024-01-01T00:00:00Z'
      }
    ...
    setTimeout(() => {
      ...
      setRecurringPatterns(mockPatterns);
      setLoading(false);
    }, 1000);
  }, [year]);
  ...
  const getLiturgicalDaysForDate = (date: Date) => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <select
            value={selectedFeastType}
            onChange={(e) => setSelectedFeastType(e.target.value)}
            className="px-4 py-2 border border-gray-300 rounded-md focus:ring-primary-500 focus:border-primary-500"
          >
          ...
          {days.map((day, idx) => {
            ...
            return (
              <div
                ...
                className={`
                  bg-white p-2 min-h-32 cursor-pointer hover:bg-gray-50
              ...
              >
                ...
                <div className="space-y-1">
                  {dayLiturgicalDays.map(liturgicalDay => (
                    <div
                      key={liturgicalDay.id}
                      onClick={(e) => {
                        e.stopPropagation();
                        setShowDayDetails(liturgicalDay);
                      }}
                      className={`
                        text-xs p-1 rounded cursor-pointer border
                        ${getSeasonColor(liturgicalDay.liturgical_season)}
                      `}
                    >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            <div key={season} className="flex items-center space-x-2">
              ...
              <div className="flex items-center">
                {icon}
                <span className="ml-1 text-sm font-medium">{season}</span>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <p className="text-gray-900">{showDayDetails.description}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-900">
                    {format(new Date(showDayDetails.date), 'MMMM dd, yyyy')}
                  </p>
                </div>
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-1">Feast Type</h4>
                  <div className="flex items-center">
                    {getFeastTypeIcon(showDayDetails.feast_type)}
                    <span className="ml-2">{showDayDetails.feast_type}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-1">Season</h4>
                  <div className="flex items-center">
                    {getSeasonIcon(showDayDetails.liturgical_season)}
                    <span className="ml-2">{showDayDetails.liturgical_season}</span>
                  </div>
                </div>
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-1">Liturgical Color</h4>
                  <div className="flex items-center">
                    <div className={`w-6 h-6 rounded-full border ${getLiturgicalColor(showDayDetails.liturgical_color)}`} />
                    <span className="ml-2">{showDayDetails.liturgical_color}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center">
                ...
                <span className="text-sm text-gray-600">
                  {showDayDetails.is_movable ? 'Movable feast' : 'Fixed feast'} • Rank: {showDayDetails.rank}
                </span>
              </div>
            </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="flex items-center mt-2 space-x-4">
                          ...
                          <span className={`text-xs px-2 py-1 rounded ${pattern.is_active
      ...
      {/* Create/Edit Liturgical Day Modal */}
      {showCreateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b">
              <h3 className="text-lg font-semibold">
                {editingDay ? 'Edit Liturgical Day' : 'Add Liturgical Day'}
              </h3>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  defaultValue={editingDay?.description}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-primary-500 focus:border-primary-500"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    defaultValue={editingDay?.feast_type}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-primary-500 focus:border-primary-500"
                  >
                    <option value="SOLEMNITY">Solemnity</option>
                    <option value="FEAST">Feast</option>
                ...
                <div>
                  ...
                  <select
                    defaultValue={editingDay?.liturgical_season}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-primary-500 focus:border-primary-500"
                  >
                    <option value="ADVENT">Advent</option>
                    <option value="CHRISTMAS">Christmas</option>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    defaultValue={editingDay?.liturgical_color}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-primary-500 focus:border-primary-500"
                  >
              ...
              <div className="flex items-center">
                <input
                  type="checkbox"
                  id="is_movable"
                  defaultChecked={editingDay?.is_movable}
                  className="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300 rounded"
                />
                <label htmlFor="is_movable" className="ml-2 block text-sm text-gray-900">
                  Movable Feast
                </label>
              </div>
            </div>
            ...
            <div className="p-6 border-t flex justify-end space-x-3">
              ...
              <button
                onClick={() => {
                  ...
                  toast.success(editingDay ? 'Liturgical day updated' : 'Liturgical day created');
                  setShowCreateModal(false);
                  setEditingDay(null);
                }}
