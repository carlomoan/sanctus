...
import { Calendar, Clock, MapPin, Users, Plus, Filter, Search, ChevronLeft, ChevronRight, MoreVertical, Edit, Trash2, Eye, RefreshCw, CalendarDays, Bell, Tag, Repeat, X, Building, Globe } from 'lucide-react';
...
const Events = () => {
  ...
  // Check if user can create diocese events
          ...
          <div className="grid grid-cols-7 gap-px bg-gray-200">
            ...
            {days.map((day, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className={`bg-white p-2 min-h-[100px] ${!isCurrentMonth ? 'text-gray-400' : ''
                    }`}
                >
                    ...
                    {dayEvents.slice(0, 2).map(event => (
                      <div
                        ...
                        className={`
                          ...
                          ${event.status === 'published' ? 'bg-primary-100 text-primary-800' : 'bg-gray-100 text-gray-800'}
                        `}
                      >
                        {event.title}
                      </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-primary-500 focus:border-primary-500"
                  disabled={!canCreateDioceseEvents}
                >
                  <option value="parish">Parish Event</option>
                  {canCreateDioceseEvents && <option value="diocese">Diocese Event</option>}
                </select>
                {!canCreateDioceseEvents && (
                  <p className="text-xs text-gray-500 mt-1">
                    Only SuperAdmin and ParishAdmin can create diocese events
                  </p>
                )}
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-900">
                    {format(new Date(showEventDetails.start_date), 'MMMM dd, yyyy')}
                  </p>
                </div>
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-1">Time</h4>
                  <p className="text-gray-900">
                    {showEventDetails.start_time || 'All day'}
                    {showEventDetails.end_time && ` - ${showEventDetails.end_time}`}
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-gray-900 flex items-center">
                  ...
                  {showEventDetails.max_participants > 0 && `/${showEventDetails.max_participants}`}
                </p>
              </div>
