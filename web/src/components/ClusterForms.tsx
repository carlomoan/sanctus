...
export function CForm({ i, p, selectedParishId, onD, onX }: { i?: Cluster; p: Parish[]; selectedParishId?: string; onD: (d: any) => Promise<void>; onX: () => void }) {
  const { register, handleSubmit, formState: { isSubmitting }, reset, setValue } = useForm();
  useEffect(() => {
    if (i) reset(i);
    else if (selectedParishId) setValue('parish_id', selectedParishId);
  }, [i, reset, selectedParishId, setValue]);
  ...
  return <form onSubmit={handleSubmit(async (data) => {
    ...
    <div className="grid grid-cols-2 gap-4">
      ...
      <div><label className="text-sm font-medium text-gray-700">Cluster</label>
        <select {...register('cluster_id')} disabled={!!i || !!selectedClusterId} className={ic + (!!selectedClusterId ? ' disabled:bg-gray-100' : '')}>
